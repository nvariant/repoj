...
##Summary of the data:
The data in this dataset contains the result of an experiment performed by Jorge L. Reyes-Ortiz, Davide Anguita, Alessandro Ghio, Luca Oneto at the Smartlab - Non Linear Complex Systems Laboratory inGenoa, Italy (1). The files and a description of the original work is found at http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones. The description of the data that follows comes from this source.

In the experiment 30 subjects wearing Samsung Galaxy S smartphone on the waist performed 6 separate activities. The smartphone was equipped with an accelerometer and took measurements during those activities. The activities were filmed and manually recorded in the dataset.
...
###subject: 
Numeric identifer for the 30 subjects who took part in the experiments. The subjects ranged in age from 19-48 years, and each performed the six activities (walking, walking_upstairs, walking_downstairs, sitting, standing, laying) while wearing the cell phone. The total number of measurements recorded for each subject ranged from 281-409.

###activity: 
This variable gives the activities performed when a given set of measurements was taken by the smartphone. This variable takes the values:
...
Here is the full set of the 561 variables, found in columns 3-565 of the data set:
tBodyAcc-mean()-X
tBodyAcc-mean()-Y
tBodyAcc-mean()-Z
tBodyAcc-std()-X
...
The train and test dataframes were then merged into one using rbind().

The 561 feature names were loaded into R and became the column names for columns 3-563. 

The data was now completely assembled, and the extract was taken by using grep() to identify the columns of data containing mean or standard deviation data.

Finally, the tidy dataset was created using lapply and data table subsetting, and a .csv file was created.
